...
1. The three validators (per-pair, flattening, anchored flattening) agree.
2. The flattening's operation counts match the structural prediction: one
   ...
   extra parent / child, one combination per node.
...
class Counters:
    ...
    def reset(self):
        self.trans = self.meet = self.comb = 0
...
def predict(environment) -> Tuple[int, int, int, int]:
    """(edge, trans, meet, comb) call counts predicted from the graph."""
    parents, children = flat.adjacency(environment)
    edges = len(environment.Es)
    meets = sum(max(0, len(parents[n.name]) - 1) for n in environment.Ns) + sum(
        max(0, len(children[n.name]) - 1) for n in environment.Ns
    )
    return (edges, 2 * edges, meets, len(environment.Ns))


def counted_run(environment, sigma) -> Tuple[int, int, int, int]:
    instrument()
    COUNTERS.reset()
    assert flat.flatten_dp(environment, sigma) is True
    edges = len(flat.edge_table(environment, sigma))
    got = (edges, COUNTERS.trans, COUNTERS.meet, COUNTERS.comb)
    deinstrument()
    return got


def main() -> None:
    print("[1] verdict agreement: pair_valid == flatten_dp == flatten_anchored")
    ok = True
    for k in (1, 2, 3, 4, 8, 16, 32):
        ...
        if not (v == flat.flatten_dp(environment, sigma)
                == flat.flatten_anchored(environment, sigma)):
            print(f"  MISMATCH at k={k}")
            ok = False
    print(f"  {'OK' if ok else 'FAIL'}")

    print("\n[2] operation counts: instrumented vs predicted (edge, trans, meet, comb)")
    ok = True
    ...
    for k in (8, 32, 128):
        ...
        flat_mean, _ = measure(lambda: flat.flatten_anchored(environment, sigma))
